...
CREATE TABLE events (
  ...
  membership_version_at_event INTEGER,      -- v3.1: 生成时从 channel_meta.membership_version 带出,fanout 便宜权限门禁
...
- **WebSocket command（v3.1 收口 message.send 幂等键）：** `message.send` 只强制 `client_message_id`（业务幂等，落 `messages.client_message_id` UNIQUE）；不再强制独立 `idempotency_key`，服务端把 `idempotency_key` 缺省映射为 `client_message_id` 写入 `idempotency_keys` 表（operation=`message.send`）。`command_id` 仍为 committed_ack 关联 id。前端发消息协议更轻（一个键，不是三个）。`command.invoke`/`interaction.submit` 仍保留 `client_invocation_id`/`client_interaction_id` 作业务幂等键，落 `invocations`/`interactions` 的 UNIQUE。
