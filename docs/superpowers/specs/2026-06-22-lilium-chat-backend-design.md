...
CREATE TABLE idempotency_keys (
  ...
  operation        TEXT NOT NULL,           -- message.send | channel.patch | invite.create | ...
...
-- 消息发送幂等也由 messages.client_message_id UNIQUE 兜底
...
**机制（平台核验合规）：** outbox 行在 source DO 业务写事务内同事务写入（单 DO 私有事务性 storage，业务写与 outbox 行原子）。一个 DO alarm flush pending 行（平台只有单 alarm，用 earliest `next_attempt_at` + 循环 re-arm；alarm at-least-once + 自动退避重试，目标 DO 写入幂等）。成功后置 `status='delivered'`。
...
- ChatChannel 产生 event 后写 outbox（target_kind=channel_fanout）→ flush 到 ChannelFanout DO → ChannelFanout 把 event 展开成对每个在线 session 的 `fanout_queue` 行 → DO alarm 批量 `fetch` 各 UserConnection DO 的 `deliver(event)`。
- **fanout 只投递在线用户**，不扫全体 members。
- **fanout 不在业务写事务里**：ack 在 ChatChannel 事务后即返回，fanout 异步。
- **delivery 失败不阻塞 command success**：失败重试至预算耗尽，最终 replay 是补偿路径。
- **member.left 撤销**：ChatChannel leave 事务写 outbox（target_kind=channel_fanout, target_key=channel_id + 该 user）→ ChannelFanout 删除该 user 的 online_sessions 行 + 丢弃其 pending fanout_queue 行。
- **权限版本门禁兜底**：deliver 前 UserConnection DO 查 target channel 当前 membership_version > 订阅版本则拒绝投递并 repair。
...
- **WebSocket command**：command frame `idempotency_key` 落 ChatChannel DO 的 `idempotency_keys` 表（operation 如 `message.send`/`command.invoke`/`interaction.submit`），与业务写同事务。`client_message_id`（消息业务幂等）落 `messages.client_message_id` UNIQUE。
...
窗口/容量配置化（如 per-user 消息 10/s + burst 20；per-channel 50/s；upload 5/min）。命中限流返回 `429 RATE_LIMITED`（`retryable: true`，带 `Retry-After`）。状态放 DO storage；per-IP WS connect 也可用 Cloudflare Rate Limiting/WAF。业务语义限流在 DO 内兜底。
...
交付：频道 CRUD + members + read-state + projection outbox flush + repair。

- channels/members 增删改 + `membership_version` + member.left 经 outbox 通知 ChannelFanout 撤销订阅。
...
该 contract 修订在本 spec 实现前由 dzmm_archive 出文档并与前端 `chatConnectionStore` 重构同步。后端阶段 1/2 实现按此修订形状，不实现原 contract 的单全局 cursor。
