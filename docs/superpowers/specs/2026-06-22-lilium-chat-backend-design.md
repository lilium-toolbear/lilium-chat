...
- **WebSocket command（v3.1 收口 message.send 幂等键；v3.2 命名空间化）：** `message.send` 只强制 `client_message_id`（业务幂等，落 `messages` 的 `UNIQUE(channel_id, dedupe_principal_key, client_message_id)`）；不再强制独立 `idempotency_key`，服务端把 `idempotency_key` 缺省映射为 `client_message_id` 写入 `idempotency_keys` 表（operation=`message.send`）。`command_id` 仍为 committed_ack 关联 id。前端发消息协议更轻（一个键，不是三个）。`dedupe_principal_key` 按 principal 命名空间化（`user:<uid>` / `bot:<bot_id>` / `system:<...>`）——不同用户用相同 `client_message_id` 各自产生不同 message，互不挡。`command.invoke`/`interaction.submit` 仍保留 `client_invocation_id`/`client_interaction_id` 作业务幂等键，落 `invocations`/`interactions` 的 UNIQUE。
