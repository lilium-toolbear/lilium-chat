...
状态：设计稿（v4.4，移除默认 system channel：无自动建频道/自动入群；所有 ChatChannel DO name=`channel_id`；Phase 7 Bot Gateway WebSocket RPC…）
日期：2026-06-22
...
D. **默认频道（v4.4 superseded）**：v1–v4.3 所有用户共享一个系统公共频道（DO name=`system-general`，bootstrap 时 lazy-create + lazy-join）。**v4.4 起取消**：不存在默认/系统公共频道；新用户 bootstrap 返回空 `channels`；用户仅通过创建频道、`POST /channels/{id}/join`、邀请、或被添加成员等方式获得 membership。
...
- **Phase 3 范围收口（§8 阶段 3，v3.4）**：阶段 3 改为"Channel CRUD + Member Management + Read State"。新增 `POST /api/chat/channels`（频道创建）：任意已认证 Browser 用户可创建 `kind="channel"` 频道，创建者=owner，可带 `initial_members`，同事务发 `channel.created`+`member.joined`+`system.notice`，DO name=`channel_id`（v4.4：无 system channel 例外）。member.left 复用 Phase 2 的 `markMemberLeftAndEnqueueFanoutUnregister`。公开目录/join/invites/DM/bot 留 Phase 6/7。与 contract v2.4 §5.2b/§12.4 对齐。
...
Phase 7 bot runtime transport 不再以 HTTP callback 为主。改为 bot 主动向 Chat 发起 outbound WebSocket（`/api/chat/bot/ws`，bot token 鉴权），Chat 经此 WS 向 bot 推 delivery（`command_invocation` / `message_interaction` / `message_event`），bot 回 `delivery_result`（含 effects），Chat 回 `delivery_ack`。HTTP callback（Chat → bot `POST <bot_callback_url>` + HMAC 签名）降级为 **future transport**，Phase 7 不实现。v4.3 收口（contract v2.10 delta）：
...
### 0.11 v4.4 修订：移除默认 system channel

v4.4 取消 Phase 1 引入的「系统公共频道」特殊路径：

- **删除 lazy-create**：移除 `POST /internal/maybe-create-system` 与 `ensureSystemChannel()`。不存在服务端自动创建的默认频道。
- **删除 lazy-join**：移除 `ensureSystemJoined()` 及 bootstrap/channels/messages/events 上的隐式入群副作用。新用户 `GET /bootstrap` 返回 `channels=[]`、`active_channel=null`（与阶段 0 空壳一致，除非用户已有 membership）。
- **统一 DO 路由**：`ChatChannel` DO name 恒等于 client `channel_id`（UUIDv7）。调用方直接使用 `getByName(channel_id)`，不再经中间 helper。
- **既有数据**：已部署环境中 DO name=`system-general` 的旧系统频道**不在运行时继续特殊路由**。运维一次性迁移：读取其 `channel_meta.channel_id`，将 SQLite 数据复制/重绑到 DO name=`channel_id` 的新实例（或让用户经公开目录重新 join 一个新频道）。迁移脚本不在本仓库 runtime 内实现。
- **公开频道发现**：原系统频道若需保留为社区入口，运维将其 `visibility` 保持 `public_listed` 并完成 DO 迁移后，用户经 `GET /channels/directory` + `POST /channels/{id}/join` 主动加入，与任意公开频道相同。
...
交付：ChatChannel 完整表 + UserDirectory projection + bootstrap + 历史分页 + per-channel cursor + projection repair。

- bootstrap 返回 per-channel cursor；**不**自动创建或加入任何频道（v4.4）。仅投影 `UserDirectory.my_channels` 中已有 membership。
