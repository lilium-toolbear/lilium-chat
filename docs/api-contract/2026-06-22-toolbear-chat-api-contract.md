...
| GET | `/api/chat/channels/{channel_id}/public-catalog` | 公开频道目录 | §5.6 |
...
| GET | `/api/chat/bot/ws` | Bot Gateway WebSocket RPC（bot token，outbound WS）(v2.8 delta) | §9.7 |
| PUT | `/api/chat/bot/commands` | Bot 注册全局 slash command catalog（bot token）(v2.8 delta) | §9.3 |
| POST | `/api/chat/bot/channels/{channel_id}/messages` | Bot 直接发消息（bot token）(v2.8 delta) | §9.8 |
| PATCH | `/api/chat/channels/{channel_id}/bot-installations/{bot_id}/event-subscriptions/message.created` | 启用/禁用 bot passive `message.created` 订阅（Browser admin）(v2.8 delta) | §9.9 |
| POST | `/api/chat/channels/{channel_id}/bot-installations` | 安装 bot 到频道（Browser admin）(v2.8 delta) | §9.3 |
| PATCH | `/api/chat/channels/{channel_id}/bot-installations/{bot_id}` | 更新 bot 安装/卸载（Browser admin）(v2.8 delta) | §9.3 |
| PATCH | `/api/chat/channels/{channel_id}/commands/{bot_command_id}` | 启用/禁用频道内 command binding（Browser admin）(v2.8 delta) | §9.3 |
| GET | `/api/chat/channels/{channel_id}/commands` | 查询当前用户可用 command（prefix suggest）(v2.8 delta) | §9.4 |
| WS | `command.invoke` / `interaction.submit` | slash command 调用 / rich UI interaction 提交（路由到 ChatChannel DO → Bot Gateway delivery）(v2.8 delta) | §9.5 / §9.6 |
...
- Bot runtime delivery 走 **Bot Gateway WebSocket RPC**：bot 主动 outbound 连 `/api/chat/bot/ws`（bot token 鉴权）→ `BotConnection DO(bot_id)`，Chat 向 bot 推 `delivery` 帧（`command_invocation` / `message_interaction` / `message_event`），bot 回 `delivery_result`（含 effects），Chat 回 `delivery_ack`（v2.8 delta）。
...
- HTTP callback（Chat → bot HTTP `POST <bot_callback_url>` + HMAC 签名）降级为 **future transport**，Phase 7 不实现（v2.8 delta）。
...
| 503 | BOT_OFFLINE | command.invoke/interaction.submit precheck 时 bot 未连接 Bot Gateway WS，可重试 (v2.8 delta) |
| 422 | BOT_EFFECT_INVALID | bot `delivery_result` 返回的 effect 校验失败（ownership/stream 不变量/components 非法）(v2.8 delta) |
| 409 | BOT_EFFECT_CONFLICT | 同一 `(channel_id, bot_id, client_effect_id)` 配不同 effect body (v2.8 delta) |
| 429 | RATE_LIMITED | 限流命中，可重试 (v2 delta) |
| 503 | BOT_CALLBACK_UNAVAILABLE | future HTTP callback transport 预留；Phase 7 Bot Gateway WS 为唯一 runtime transport，此码不返回 (v2.8 收窄) |
...
## 14. v2.8 addendum 实现不变量（Phase 7 Bot Gateway WS RPC）
