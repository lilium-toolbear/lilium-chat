...
状态：实现前 API contract（v2.1，基于 2026-06-21 v1 + backend 设计 v3.2 delta + 2026-06-23 成员精确读补丁）
日期：2026-06-22
范围：lilium-chat 后端（Cloudflare Worker + Durable Object）的 browser/bot-facing wire shape
权威来源：

- 实现设计：`docs/superpowers/specs/2026-06-22-lilium-chat-backend-design.md`（v3.2）
...
该 endpoint 的 URL 不含 `channel_id`，服务端内部按 `invite_code` 定位频道 (v2 delta)。索引 lag 窗口内返回 `409 ROUTE_INDEX_PENDING`。
...
### 12.4 阶段 3：频道与成员管理 + read-state

### 12.5 阶段 4：消息生命周期

回复 + 编辑 + 撤回 + 管理员删除（含 `ROUTE_INDEX_PENDING` 行为）。
...
不进 lilium-chat。每后端阶段交付后，前端在 dzmm_archive 对应接入。前端开工前必须落地本 contract v2（per-channel cursor + committed_ack + 幂等简化 + ROUTE_INDEX_PENDING），否则前后端会引用不同 contract (v2 delta)。
