...
状态：实现前 API contract（v2.4，基于 2026-06-21 v1 + backend 设计 v3.3 delta + 2026-06-23 成员精确读补丁 + 2026-06-24 前端缺口收口 + 2026-06-24 幂等冲突语义收口 + 2026-06-24 频道创建端点）
日期：2026-06-22
...
| `message.send` 幂等键 | 同时要求 `command_id` + `idempotency_key` + `payload.client_message_id` | **只强制 `payload.client_message_id`**；`idempotency_key` 缺省映射为 `client_message_id`，`command_id` 降级为 ack 关联 id (v2 delta) |
| `command.invoke` / `interaction.submit` 幂等 | `idempotency_key` | 保留 `idempotency_key`，或用 `client_invocation_id` / `client_interaction_id` 作业务幂等键 (v2 delta) |
| 路由索引 lag 行为 | 未定义 | `/messages/{id}`、`/invites/{code}` 在索引 lag 窗口返回 **`409 ROUTE_INDEX_PENDING`** (v2 delta) |
...
HTTP mutation 使用 header：

```http
Idempotency-Key: <client-generated-key>
```

WebSocket command 的幂等按 command 类型区分（v2 delta）：

- `message.send`：业务幂等键是 `payload.client_message_id`。command frame 不强制独立 `idempotency_key`；若客户端不传，服务端把 `idempotency_key` 缺省映射为 `client_message_id`。`command_id` 仅作 ack 关联 id，不参与幂等判定。
...
- command request 带 `command_id`（ack 关联）和业务幂等键（见 2.5）(v2 delta)。
...
```json
{
  ...
  "client_message_id": "00000000-0000-7000-8000-000000000401",
  "channel_id": "00000000-0000-7000-8000-000000000201",
  "sender": {
    "kind": "user",
    "user": {
...
```http
POST /api/chat/channels/{channel_id}/read-state
Idempotency-Key: client-key-read-state
```
...
WebSocket command frame (v2 delta：只强制 `client_message_id`，不再要求 `idempotency_key`)：
...
```json
{
  ...
  "channel_id": "00000000-0000-7000-8000-000000000201",
  "payload": {
    "client_message_id": "00000000-0000-7000-8000-000000000402",
    "type": "image",
    "text": "",
    "reply_to_message_id": null,
...
Idempotency-Key: client-key-message-edit
```

该 endpoint 的 URL 不含 `channel_id`，服务端内部按 `message_id` 定位频道 (v2 delta)。索引 lag 窗口内返回 `409 ROUTE_INDEX_PENDING`。committed_ack 已携带 `message_id` + `channel_id`，前端可走 channel-scoped 路径避免短窗口重试。
...
Idempotency-Key: client-key-message-recall
```
...
Idempotency-Key: client-key-message-delete
```
...
```json
{
  "commands": [
    {
      "command_id": "00000000-0000-7000-8000-000000000701",
      "name": "ask",
      "enabled": true,
      "updated_at": "2026-06-21T05:30:00Z"
    }
...
同一频道内 enabled command 名称不能冲突。冲突返回 `COMMAND_NAME_CONFLICT`。
...
WebSocket command frame (v2 delta：保留 `idempotency_key` 或用 `client_invocation_id` 作业务幂等键)：
...
```json
{
  ...
  "idempotency_key": "client-key-command-invoke",
  "channel_id": "00000000-0000-7000-8000-000000000201",
  "payload": {
    "chat_command_id": "00000000-0000-7000-8000-000000000701",
    "client_invocation_id": "00000000-0000-7000-8000-000000000801",
    "options": {
      "prompt": {
        "type": "string",
        "value": "hello"
...
WebSocket command frame (v2 delta 标注幂等键来源)：
...
```json
{
  ...
  "idempotency_key": "client-key-component-interaction",
  "channel_id": "00000000-0000-7000-8000-000000000201",
  "payload": {
    "message_id": "00000000-0000-7000-8000-000000000301",
    "client_interaction_id": "00000000-0000-7000-8000-000000000a11",
    "component_id": "00000000-0000-7000-8000-000000000a01",
    "custom_id": "confirm",
    "value": true
  }
...
```json
{
  ...
  "idempotency_key": "client-key-component-interaction-select",
  "channel_id": "00000000-0000-7000-8000-000000000201",
  "payload": {
    "message_id": "00000000-0000-7000-8000-000000000301",
    "client_interaction_id": "00000000-0000-7000-8000-000000000a12",
    "component_id": "00000000-0000-7000-8000-000000000a02",
    "custom_id": "pick_item",
    "value": "sword"
  }
...
```json
{
  "client_message_id": "00000000-0000-7000-8000-000000000902",
  "type": "text",
  "text": "system notice",
  "reply_to_message_id": null,
  "attachment_ids": [],
...
| 409 | IDEMPOTENCY_CONFLICT | 同一 Idempotency-Key 请求体不一致 |
| 409 | ROUTE_INDEX_PENDING | 索引 lag 窗口，重试可路由到 (v2 delta) |
...
目标：8 个 DO 类壳 + JWT 自验 + Hyperdrive profile resolve + 部署 chat.kuma.homes + 平台 spike 跑通（hibernation、Hyperdrive、SeaweedFS、replay-after-delete、message_id/invite_code 路由、单 alarm earliest-wins、projection outbox flush）。
...
频道创建、读取、更新、解散；成员列表、精确读、添加、角色修改、移除/退出；read-state；相关事件包含 `channel.created`、`channel.updated`、`channel.dissolved`、`member.*`、`read_state.updated`、`system.notice`。

不在本阶段：公开目录 / discovery、`POST /channels/{id}/join`、invite create/accept、DM 创建、bot command（留 Phase 6/7）。

### 12.5 阶段 4：消息生命周期

回复 + 编辑 + 撤回 + 管理员删除（含 `ROUTE_INDEX_PENDING` 行为）；管理员删除他人消息追加 `system.notice`。
...
不进 lilium-chat。每后端阶段交付后，前端在 dzmm_archive 对应接入。前端开工前必须落地本 contract v2.2（per-channel cursor + committed_ack + 幂等简化 + ROUTE_INDEX_PENDING + dissolve + system.notice + 资源级 not-found code），否则前后端会引用不同 contract (v2 delta)。
