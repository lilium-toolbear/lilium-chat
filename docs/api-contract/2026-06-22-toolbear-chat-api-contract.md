...
状态：实现前 API contract（v2.11，v4.4-aligned —— … + 2026-06-27 Phase 8 Live Fanout：`session.live_start` 全频道 live push（无 WS replay/cursor）、`lilium.chat.v2`、HTTP 权威恢复）
日期：2026-06-22
...
| Live push 订阅模型 | connect 后 server 自动 register-online + replay | **`session.live_start` + `session.heartbeat`**；全 active 频道 best-effort live push；`ChannelFanout` 为 TTL lease cache；**v1 无 `channel.subscribe`** (v2.11 delta) |
...
- **v2.11 (2026-06-27)**：Phase 8 Live Fanout redesign（无 WS cursor recovery）。具体：Browser WS 子协议 **`lilium.chat.v1` → `lilium.chat.v2`（breaking）**；§10.1 connect 只建 live session，**不** replay、**不**解析 `?cursors=`、**不**隐式订阅；新增 §5.11 `session.live_start`（WS open 后自动发送，为全部 active 成员频道注册 fanout lease，**不** replay 历史事件）、§5.12 `session.heartbeat`（刷新 lease TTL）；§10.5 WS live delivery 语义（best-effort）+ §10.6 HTTP 权威恢复；**v1 不暴露 `channel.subscribe` / `channel.unsubscribe`**；§5.5 明确 `channel.mark_read` 的 `last_read_event_id` 不是 WS delivery cursor；新增 `GET /api/chat/channels/{channel_id}/events`（§6.1b）；§12.11 Phase 8；§15 v2.11 addendum。设计权威：`docs/plans/2026-06-27-userconnection-live-subscription-redesign.md`。
...
- 刷新本 session 全部 active fanout lease 的 `expires_at`（并 upsert 到 `ChannelFanout`）。
