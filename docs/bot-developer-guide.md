...
| `chat:messages:write` | `POST /api/chat/bot/channels/{channel_id}/messages`、delivery effects 中的 `send_message` 等 |
...
`message` 为完整 Browser 可见投影（含 `sender.kind=bot`）。

**约束**：

- Bot 必须已安装在该频道且 `status=active`。
- 可携带 `components`（见 §7）。
- DM 频道返回 `409 UNSUPPORTED_CHANNEL_KIND`。

**实现状态**：⏳ Contract 已定义，**路由尚未注册**（`index.ts` 无此 handler）。当前应通过 Bot Gateway `delivery_result` effects 发消息（effects 管线亦在开发中）。
