...
状态：实现前 API contract（v2.25，v4.4-aligned —— … + 2026-06-27 DM §5.2c；+ 2026-06-30 Bot streaming §9.13–§9.16）
...
Bot stream **正常完成**：`stream_state=final`、`status=normal`。Bot stream **中断保留 partial**：`stream_state=abandoned`、`status=failed`；**不是**正常完成回答，前端须与 final bot reply 视觉区分。Abandoned partial **不**附带 finalize-only `components` / `attachment_ids`（除非已在 finalize 路径验证并提交）。
