...
状态：实现前 API contract（v2.24，v4.4-aligned —— … + 2026-06-27 DM §5.2c；+ 2026-06-30 Bot streaming §9.13–§9.16）
...
- **`finalize_request_hash`** = 稳定 hash（如 SHA-256 hex）over 规范化 JSON `{ final_seq, resolved_text, components, attachment_ids }`（缺省字段按 canonical JSON 规则一致化）；**幂等判断用此 hash**。
