...
状态：实现前 API contract（v2.28，v4.4-aligned —— … + 2026-06-27 DM §5.2c；+ 2026-06-30 Bot streaming §9.13–§9.16；+ 2026-06-30 Rich UI interaction lifecycle §9.5/§9.6；+ 2026-06-30 Bot markdown §3.9）
...
`format=markdown` 只允许 Bot 消息使用。普通用户消息固定为 `format=plain`。Browser 渲染规则见 **§3.9**。
...
- Bot **streaming** 路径（`start_stream` → Stream WS `append` / `finalize`；`stream_state` 为 `streaming` | `final` | `abandoned`）**不得**携带、持久化或向 Browser 投影非空 `components`。Stream 消息正文仅为 `text`（`format=plain` | `markdown`）。
...
### 3.9 Bot Markdown 渲染 (v2.28 delta)

`format=markdown` 的 Bot 消息由 Browser 使用 **markdown-it + DOMPurify** 渲染（`Message.text` 为 Markdown 源码；**不**使用 `v-html` 直出未净化 HTML）。

**适用范围：**

- 仅 `sender.kind=bot` 且 `format=markdown` 且消息内容可见（`status` 为 `normal` / `edited` / `failed` 等可展示正文的状态）。
- `stream_state=streaming` 期间按 **纯文本** 追加展示（不解析 Markdown，避免半截语法闪烁）；`stream_state` 变为 `final` / `abandoned` 且 `format=markdown` 时一次性完整渲染。

**支持的 Markdown 语法（Bot 可依赖）：**
...
| `[文字](url)` | 链接（见下方链接规则） |
| `![alt](url)` | 图片（见下方图片规则） |
| 受限 inline HTML | 经 DOMPurify 白名单保留（如 `span`）；危险标签剔除 |

**链接规则：**

| URL 类型 | Browser 行为 |
|---|---|
| 站内（`*.kuma.homes` 或相对路径 `/...`）且协议为 `http:`/`https:` | 渲染为可点击 `<a href>`；`target="_blank"` + `rel="noopener noreferrer"` |
| 站外 `http:`/`https:` | 渲染为 **不可点击** 的 `<span>`（**无** `href`）；可见文字可选取复制；`title` 保留完整 URL 便于复制 |
| `javascript:` / `data:` / `vbscript:` 等 | 阻断，不生成可导航链接 |

**图片规则：**

- `http:`/`https:` 图片 **均允许加载**（含站外 CDN）；渲染为 `<img>`。
- 所有图片带 `referrerpolicy="no-referrer"` + `loading="lazy"`（不向图片源发送 referrer）。
- `javascript:` / `data:` 等危险 `src` 阻断，降级为纯文本。

**HTML 安全：**

- Markdown 内联 HTML **允许**经净化后的安全标签（如 `span`、`p`、`strong`）。
- `script`、`iframe`、`object`、`embed`、`form` 等危险标签与事件属性由 DOMPurify 剔除。
- Bot **不应**依赖 raw HTML 作为首选格式；Markdown 为主，HTML 为补充。

**Slash command chip 虚拟链接（platform 输出约定）：**

Platform `/help`、`/permission` 等内置命令在 bot markdown 中引用 slash 命令时，使用 **虚拟 href** 而非真实 URL：

```markdown
[`/help`](/command:help) — 查看可用命令
```

- 语法：`[`显示文字`](/command:<canonical_name>)`（`<canonical_name>` 为命令 canonical name，不含前导 `/`）。
- Browser 将 `/command:*` 渲染为 **可点击 chip**（视觉类似 @mention）；点击打开该命令的帮助面板（manifest lookup），**不**导航离开页面。
- 第三方 Bot **可以**复用该约定；Browser 对任意 bot markdown 中的 `/command:<name>` 链接同等处理。
...
`/help` / `/permission` 响应以 `format=markdown` 的 bot text message 写入频道 timeline（§3.9）；`messages` 行持久化 `sender_bot_display_name` / `sender_bot_avatar_url`，Browser 投影为完整 `sender.bot`（不查 BotRegistry）。
...
- `send_message`: 发送 **非 stream** Bot 消息（canonical `stream_state=none`）；可含 `components`（§3.8）。
...
- `start_stream`: 创建 streaming registry + 返回 Stream WS URL（**不**写入 canonical `messages`；**禁止** `components`；详见 §9.13–§9.14）。
...
`start_stream` effect body：允许 `message.type`、`message.format`、`reply_to_message_id`（或等价 reply 字段）。**禁止**非空 `message.components`（→ `422 BOT_EFFECT_INVALID`）。`message.text` 为空或忽略。`attachment_ids` 当前 **禁止**（与 §9.15.4 finalize 一致）。语义：创建 `message_stream_registry`；**不**插入 canonical `messages`；emit live-only `message.stream_started`；Bot 须在 `expires_at` 前连接 Stream WS。
...
### 9.17 预留能力（future / open）

**Owner API：** §9.10/§9.11 继续用 Browser JWT；本阶段 **不**接受 Machine Token 于 `/api/chat/bots*`。

**Read scopes：** `chat:messages:read` / `chat:channels:read` / `chat:members:read` 已定义 scope，**无** Bot Token read endpoint。

**Attachments：** 若实现，channel-scoped `POST .../uploads/images/presign|finalize`；需 `chat:attachments:write`；v1 不可跨 channel 复用。
...
列表与变更确认消息使用 `format=markdown`（§3.9）：标题/分组用标准 Markdown；命令名使用 slash command chip 虚拟链接 `` [`/name`](/command:name) ``。
...
14. Machine Token owner API、Bot read API、Bot attachment upload 在本阶段 **不**实现，除非 implementation plan 显式纳入。
