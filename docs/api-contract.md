...
状态：实现前 API contract（v2.23，v4.4-aligned —— … + 2026-06-27 DM §5.2c；+ 2026-06-30 Bot streaming §9.13–§9.16）
...
- `start_stream`: 创建 streaming registry + 返回 Stream WS URL（**不**写入 canonical `messages`；详见 §9.13–§9.14）。
...
Bot 消息须满足与 §9.7.3 / §9.13 相同的 effect 校验：目标频道为已 allow 的 `kind=channel` 群聊、scope 含 `chat:messages:write`、components 规则同 Browser 可见消息投影。
...
| D6 | Stream 中断不 promote partial text | 只有 `finalize` 产生最终消息 |
...
`start_stream` effect body：`message.type/format/reply/components/attachment_ids` 等同普通 bot 消息 metadata；`message.text` 为空或忽略。语义：创建 `message_stream_registry`；**不**插入 canonical `messages`；emit live-only `message.stream_started`；Bot 须在 `expires_at` 前连接 Stream WS。
...
- `final_seq >= received_seq`（或 server 先 drain pending accepted deltas）。
- `finalize` 一次 canonical 事务：insert final `messages` + `message.stream_finalized` event + mark registry；**不**额外 emit `message.created`。
- registry 持久化 `final_event_id` / `final_text_hash` / `finalized_response_json` 以支持 finalize 幂等重放。

#### 9.15.5 Interruption policy

- Stream WS / 主 Bot Gateway 断连 **不** finalize；bot 可在 `expires_at` 前重连。
- Expiry 后未 finalize → abandon + live-only `message.stream_abandoned`；**不** promote partial text。
...
Browser：按 `(channel_id, message_id, stream_seq)` 去重；**不**把 stream frames 存为 channel event cursor；HTTP history/events 为权威。
...
> **Live-only stream frames**：`message.stream_started`、`message.stream_delta`、`message.stream_abandoned` **不是** channel timeline event，不写入 `ChatChannel.events`，不进入 HTTP `GET .../events` 恢复。它们经 Browser WS 以 `frame_type="stream_event"` 投递（§9.16）。离线客户端只通过 HTTP history/events 看到已 finalize 的消息。
...
8. Stream expiry/abandon **不** promote partial text。
...
11. Finalize 幂等：registry 持久化 `final_event_id` / `final_text_hash` / `finalized_response_json`；hash 不同 → `BOT_STREAM_CONFLICT`。
12. Live stream fanout 走 `ChannelFanout /internal/deliver-stream-frame`；**不得**复用 canonical `/deliver`。
13. Machine Token owner API、Bot read API、Bot attachment upload 在本阶段 **不**实现，除非 implementation plan 显式纳入。
