...
function loadInteractionRow(host: ChatChannelHost, interactionId: string): InteractionRow | null {
...
export function finalizeInteractionDelivery(
  ...
  const interaction = loadInteractionRow(host, input.interactionId);
  ...
  if (input.success) {
    const messageRow = loadMessageContextRow(host, input.channelId, interaction.message_id);
    ...
    insertInteractionCompletedEvent(host, {
      eventId,
  ...
  insertInteractionFailedEvent(host, {
    eventId,
    channelId: input.channelId,
    botId: input.botId,
    commandId: interaction.command_id,
  ...
  appendChatChannelArchive(host.ctx, input.channelId, input.now, [eventId], () =>
    collectDefinedChanges([upsertEventChange(host.ctx.storage.sql, eventId)]),
  );
  return eventId;
}
