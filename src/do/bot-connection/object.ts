...
interface EnqueueDeliveryResult {
  delivery_id: string;
  status: string;
}
...
// bot_delivery_outbox rows through enqueueDelivery(); BotConnection persists a
// bot_deliveries row, pushes a `delivery` frame, and routes the bot's
// `delivery_result` back to the source ChatChannel.
export class BotConnection extends DurableObject<Env> {
  ...
  async enqueueDelivery(botId: string, body: EnqueueDeliveryInput): Promise<EnqueueDeliveryResult> {
  ...
  async upsertStatefulSessionRef(input: {
    session_id: string;
    channel_id: string;
    bot_id: string;
    status: string;
  ...
  private async trySendDelivery(
    ...
    try {
      ...
      logSwallowedError("bot_delivery_ws_send_failed", err, { delivery_id: deliveryId });
  ...
  private async flushPendingAndSentDeliveries(
    ...
    for (const raw of rows) {
      ...
      try {
        ...
        logSwallowedError("bot_delivery_ws_send_failed", err, { delivery_id: row.delivery_id });
        this.handleDeliverySendFailure(row, String(Date.now()));
      }
    }
  }
    ...
    try {
      ...
      if (frame.type === "hello") {
        ...
        await resumeStatefulSessions(this.ctx, this.env, attachment.bot_id, (frame: SessionInputFrame) => {
          ...
          try {
            ...
            logSwallowedError("bot_stateful_session_resume_send_failed", err, { bot_id: attachment.bot_id });
            return false;
          }
        });
        return;
      ...
      if (frame.type === "session.input_ack") {
        ...
        const ref = this.ctx.storage.sql
          ...
          .toArray()[0] as { channel_id: string } | undefined;
        if (ref) {
          await notifyBotSessionInputAck(
            this.env,
            ref.channel_id,
            parsed,
          );
      ...
      logSwallowedError("bot_gateway_frame_parse_failed", err);
    }
  }
  ...
  async handleDeliveryResult(
    ...
    try {
      applyBody = await channelStub.botDeliveryResult({
      ...
      const message = err instanceof Error ? err.message : "effect application failed";
      this.ctx.storage.sql.exec(
        "UPDATE bot_deliveries SET status='failed', updated_at=?, next_attempt_at=? WHERE delivery_id=? AND status IN ('pending', 'sent')",
        now,
        String(Date.now()),
      ...
      ws.send(
        JSON.stringify(
          buildDeliveryAck(parsed.delivery_id, "failed", {
            error: { code: "CHAT_WORKER_UNAVAILABLE", message },
          }),
        ),
      );
      return;
  ...
  private async fetchBotIsOfficial(botId: string): Promise<boolean> {
    try {
      const { bot } = await this.env.BOT_REGISTRY.getByName("registry").getBot(botId);
      return bot.visibility === "official";
    } catch (err) {
      logSwallowedError("bot_official_lookup_failed", err, { bot_id: botId });
      return false;
    }
  }
